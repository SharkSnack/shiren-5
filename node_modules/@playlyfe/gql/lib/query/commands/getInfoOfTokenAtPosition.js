...
_schema, sourceText, position, config) {
  ...
  if (kind === 'Field' || kind === 'AliasedField') {
    ...
    var _type = (0, _GQLTypes.getNamedType)(fieldDef.type);
    if (_type) {
  ...
  if (kind === 'Argument') {
    ...
    if (argDef) {
      var _contents = [];
      _contents.push(argDef.print());
      var _type2 = (0, _GQLTypes.getNamedType)(argDef.type);
      if (_type2) {
  ...
  if (kind === 'ObjectField') {
    if (typeInfo.objectFieldDefs) {
      ...
      var _contents2 = [];
      if (objectField) {
        _contents2.push(objectField.print());
        var _type3 = (0, _GQLTypes.getNamedType)(objectField.type);
        if (_type3) {
          _contents2.push(_type3.print());
        }
      }
      return { contents: _contents2 };
    }
  }
