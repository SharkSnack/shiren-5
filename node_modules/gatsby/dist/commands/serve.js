...
module.exports = async program => {
  ...
  try {
    functions = JSON.parse(_fsExtra.default.readFileSync(_path.default.join(compiledFunctionsDir, `manifest.json`)));
  } catch (e) {// ignore
  }

  if (functions) {
    app.use(`/api/*`, (0, _multer.default)().none(), _express.default.urlencoded({
      extended: true
    }), (req, res, next) => {
      ...
      if (functionObj) {
        ...
        const end = Date.now();
        console.log(`Executed function "/api/${functionObj.apiRoute}" in ${end - start}ms`);
        return;
      } else {
        next();
      }
